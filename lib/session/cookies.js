  ...
  response.end = function () {
    async.map(responseCookies, function (cookie, callback) {
      ...
      for (var option in cookie.options) {
        ...
        var optionValue = cookie.options[option];
        if (optionValue === true) {
      ...
      getHmac(encodedName + encodedValue, function (err, hmac) {
        callback(null, encodedName + '=' + encodedValue + '.' + hmac + ((headerComponents.length == 0) ? '' : '; ' + headerComponents.join('; ')));
      });
      ...
      endResponse.apply(response, arguments);
    });
  };
    ...
    callback(null, requestCookies, function (name, value, suppliedOptions) {
      ...
      if (suppliedOptions) {
        for (var option in suppliedOptions) {
          options[option] = suppliedOptions[option];
        }
      }
